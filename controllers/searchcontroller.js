...
/*
** Sends back an empty result file.
*/
    ...
    if (type == "xml") {
        ...
        for (i in data) {
            ...
            var tab = {
                ...
                "match" : {
                    "title": getMatch(cityname, data[i].name, data[i].names),
                    "country" : getMatch(countryname, data[i].countryName)
                }
            };
            if (data[i].name.indexOf(title_match) != 0)
                tab.title_alt = getAltTitle(data[i].names, cityname);
            for (j in result) {
                if (data[i].adminCode == result[j].code) {
                    data[i].adminName = result[j].name;
                }
...
module.exports = function(app, express, vars) {
    app.get('/search', function(req, res){
        var city_lower, countryname, cityname, my_body, sort, ord, type = "";
