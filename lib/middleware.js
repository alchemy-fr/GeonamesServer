...
module.exports = {
    ...
    setConnectionStrings: function (app) {
        return function(req, res, next) {
            var elasticSearchConfig = app.get('app.config').es;
            var mongoConfig = app.get('app.config').mongo;

            app.set('es.connection.string', function(node) {
                return common.getElasticSearchEndpoint(
                    elasticSearchConfig.elastic_scheme,
                    elasticSearchConfig.elastic_host,
                    elasticSearchConfig.elastic_port,
                    elasticSearchConfig.elastic_cluster,
                    node
                );
            });
            ...
            app.set('mongo.connection.string', function() {
                return common.getMongoConnectionString(
                    mongoConfig.mongo_host,
                    mongoConfig.mongo_port,
                    mongoConfig.mongo_user,
                    mongoConfig.mongo_pass,
                    mongoConfig.mongo_database
                );
            });
