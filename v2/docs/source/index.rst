...
You can do all of it in just one step with the utility located in the 
**import** folder.

If your MongoDB database isn't installed on *localhost:9200*, you migh want to 
change this value in **import/import.sh** and in **import/scripts/vars.php.** 

Make sure that mongodb is running, then just run:

.. code-block:: bash
   
   sh import.sh

It will download the neccessary files from the geonames servers, format them
...
(such as **npm**), or build it using the following commands (`source <http://github.com/kuno/GeoIP>`_):
...
Attention, though we provide a **GeoLiteCity.dat** in the data folder, it 
